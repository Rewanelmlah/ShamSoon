import 'package:easy_localization/easy_localization.dart';

abstract class LocaleKeys {
  static const String _skip = 'skip';
  static String get skip => _skip.tr();

  static const String _truckyApp = 'trucky_app';
  static String get truckyApp => _truckyApp.tr();

  static const String _mapDescription = 'map_description';
  static String get mapDescription => _mapDescription.tr();

  static const String _notExist = 'not_exist';
  static String get notExist => _notExist.tr();

  static const String _english = 'english';
  static String get english => _english.tr();

  static const String _pleaseFillCodeFields = 'please_fill_code_fields';
  static String get pleaseFillCodeFields => _pleaseFillCodeFields.tr();

  static const String _closed = 'closed';
  static String get closed => _closed.tr();

  static const String _restaurant = 'restaurant';
  static String get restaurant => _restaurant.tr();

  static const String _sR = 's_r';
  static String get sR => _sR.tr();

  static const String _doYouWantToDeleteAllNotifications = 'do_you_want_to_delete_all_notifications';
  static String get doYouWantToDeleteAllNotifications => _doYouWantToDeleteAllNotifications.tr();

  static const String _doYouWantToDeleteThisNotification = 'do_you_want_to_delete_this_notification';
  static String get doYouWantToDeleteThisNotification => _doYouWantToDeleteThisNotification.tr();

  static const String _enter = 'enter';
  static String get enter => _enter.tr();

  static const String _quit = 'quit';
  static String get quit => _quit.tr();

  static const String _pleaseSelectTheProfileImage = 'please_select_the_profile_image';
  static String get pleaseSelectTheProfileImage => _pleaseSelectTheProfileImage.tr();

  static const String _pleaseEnterPhoneNumber = 'please_enter_phone_number';
  static String get pleaseEnterPhoneNumber => _pleaseEnterPhoneNumber.tr();

  static const String _newRegisteration = 'new_registeration';
  static String get newRegisteration => _newRegisteration.tr();

  static const String _doNotReceiveTheCode = 'do_not_receive_the_code';
  static String get doNotReceiveTheCode => _doNotReceiveTheCode.tr();

  static const String _pleaseFillTheFollowingData = 'please_fill_the_following_data';
  static String get pleaseFillTheFollowingData => _pleaseFillTheFollowingData.tr();

  static const String _agreeToTheTermsAndConditions = 'agree_to_the_terms_and_conditions';
  static String get agreeToTheTermsAndConditions => _agreeToTheTermsAndConditions.tr();

  static const String _conditionsAndTerms = 'conditions_and_terms';
  static String get conditionsAndTerms => _conditionsAndTerms.tr();

  static const String _searchResult = 'search_result';
  static String get searchResult => _searchResult.tr();

  static const String _searchLocation = 'search_location';
  static String get searchLocation => _searchLocation.tr();

  static const String _cannotDetectYourLocation = 'cannot_detect_your_location';
  static String get cannotDetectYourLocation => _cannotDetectYourLocation.tr();

  static const String _forbidden = 'forbidden';
  static String get forbidden => _forbidden.tr();

  static const String _pleaseEnterYourLocation = 'please_enter_your_location';
  static String get pleaseEnterYourLocation => _pleaseEnterYourLocation.tr();

  static const String _thisAccountIsNotActivatedTheActivationCodeHasBeenSent = 'this_account_is_not_activated_the_activation_code_has_been_sent';
  static String get thisAccountIsNotActivatedTheActivationCodeHasBeenSent => _thisAccountIsNotActivatedTheActivationCodeHasBeenSent.tr();

  static const String _mandatory = 'mandatory';
  static String get mandatory => _mandatory.tr();

  static const String _messageContent = 'message_content';
  static String get messageContent => _messageContent.tr();

  static const String _about = 'about';
  static String get about => _about.tr();

  static const String _privacyPolicy = 'privacy_policy';
  static String get privacyPolicy => _privacyPolicy.tr();

  static const String _city = 'city';
  static String get city => _city.tr();

  static const String _searchFilteration = 'search_filteration';
  static String get searchFilteration => _searchFilteration.tr();

  static const String _termsAndConditionsMustBeAgreedTo = 'terms_and_conditions_must_be_agreed_to';
  static String get termsAndConditionsMustBeAgreedTo => _termsAndConditionsMustBeAgreedTo.tr();

  static const String _hello = 'hello';
  static String get hello => _hello.tr();

  static const String _rates = 'rates';
  static String get rates => _rates.tr();

  static const String _message = 'message';
  static String get message => _message.tr();

  static const String _from = 'from';
  static String get from => _from.tr();

  static const String _to = 'to';
  static String get to => _to.tr();

  static const String _restaurantState = 'restaurant_state';
  static String get restaurantState => _restaurantState.tr();

  static const String _mainCategories = 'main_categories';
  static String get mainCategories => _mainCategories.tr();

  static const String _whatsappNumber = 'whatsapp_number';
  static String get whatsappNumber => _whatsappNumber.tr();

  static const String _description = 'description';
  static String get description => _description.tr();

  static const String _workDates = 'work_dates';
  static String get workDates => _workDates.tr();

  static const String _products = 'products';
  static String get products => _products.tr();

  static const String _offers = 'offers';
  static String get offers => _offers.tr();

  static const String _complaintTitle = 'complaint_title';
  static String get complaintTitle => _complaintTitle.tr();

  static const String _complaintText = 'complaint_text';
  static String get complaintText => _complaintText.tr();

  static const String _pickImage = 'pick_image';
  static String get pickImage => _pickImage.tr();

  static const String _newReports = 'new_reports';
  static String get newReports => _newReports.tr();

  static const String _processing = 'processing';
  static String get processing => _processing.tr();

  static const String _finished = 'finished';
  static String get finished => _finished.tr();

  static const String _noTitle = 'no_title';
  static String get noTitle => _noTitle.tr();

  static const String _noDescription = 'no_description';
  static String get noDescription => _noDescription.tr();

  static const String _complaintNumber = 'complaint_number';
  static String get complaintNumber => _complaintNumber.tr();

  static const String _complaintStatus = 'complaint_status';
  static String get complaintStatus => _complaintStatus.tr();

  static const String _viewAll = 'view_all';
  static String get viewAll => _viewAll.tr();

  static const String _iVisitedTheRestaurant = 'i_visited_the_restaurant';
  static String get iVisitedTheRestaurant => _iVisitedTheRestaurant.tr();

  static const String _categories = 'categories';
  static String get categories => _categories.tr();

  static const String _serviceProviders = 'service_providers';
  static String get serviceProviders => _serviceProviders.tr();

  static const String _home = 'home';
  static String get home => _home.tr();

  static const String _notifications = 'notifications';
  static String get notifications => _notifications.tr();

  static const String _addComplaint = 'add_complaint';
  static String get addComplaint => _addComplaint.tr();

  static const String _profile = 'profile';
  static String get profile => _profile.tr();

  static const String _attachments = 'attachments';
  static String get attachments => _attachments.tr();

  static const String _managementResponse = 'management_response';
  static String get managementResponse => _managementResponse.tr();

  static const String _noResponseYet = 'no_response_yet';
  static String get noResponseYet => _noResponseYet.tr();

  static const String _sentSuccessfully = 'sent_successfully';
  static String get sentSuccessfully => _sentSuccessfully.tr();

  static const String _wishGoodTimeForYou = 'wish_good_time_for_you';
  static String get wishGoodTimeForYou => _wishGoodTimeForYou.tr();

  static const String _whoUs = 'who_us';
  static String get whoUs => _whoUs.tr();

  static const String _commonQuestions = 'common_questions';
  static String get commonQuestions => _commonQuestions.tr();

  static const String _howTruckyWork = 'how_trucky_work';
  static String get howTruckyWork => _howTruckyWork.tr();

  static const String _contactUs = 'contact_us';
  static String get contactUs => _contactUs.tr();

  static const String _reports = 'reports';
  static String get reports => _reports.tr();

  static const String _setting = 'setting';
  static String get setting => _setting.tr();

  static const String _logOut = 'log_out';
  static String get logOut => _logOut.tr();

  static const String _language = 'language';
  static String get language => _language.tr();

  static const String _changeProfile = 'change_profile';
  static String get changeProfile => _changeProfile.tr();

  static const String _saveChanges = 'save_changes';
  static String get saveChanges => _saveChanges.tr();

  static const String _more = 'more';
  static String get more => _more.tr();

  static const String _pleaseEnterTheVerificationCodeSentToTheMobileNumber = 'please_enter_the_verification_code_sent_to_the_mobile_number';
  static String get pleaseEnterTheVerificationCodeSentToTheMobileNumber => _pleaseEnterTheVerificationCodeSentToTheMobileNumber.tr();

  static const String _cannotAccessLocation = 'cannot_access_location';
  static String get cannotAccessLocation => _cannotAccessLocation.tr();

  static const String _confirmLocation = 'confirm_location';
  static String get confirmLocation => _confirmLocation.tr();

  static const String _verificationCode = 'verification_code';
  static String get verificationCode => _verificationCode.tr();

  static const String _submit = 'submit';
  static String get submit => _submit.tr();

  static const String _signUp = 'sign_up';
  static String get signUp => _signUp.tr();

  static const String _email = 'email';
  static String get email => _email.tr();

  static const String _placeRate = 'place_rate';
  static String get placeRate => _placeRate.tr();

  static const String _yourEvaluationMattersToUs = 'your_evaluation_matters_to_us';
  static String get yourEvaluationMattersToUs => _yourEvaluationMattersToUs.tr();

  static const String _signIn = 'sign_in';
  static String get signIn => _signIn.tr();

  static const String _welcome = 'welcome';
  static String get welcome => _welcome.tr();

  static const String _confirm = 'confirm';
  static String get confirm => _confirm.tr();

  static const String _pleaseEnterTheCodeSentTo = 'please_enter_the_code_sent_to';
  static String get pleaseEnterTheCodeSentTo => _pleaseEnterTheCodeSentTo.tr();

  static const String _register = 'register';
  static String get register => _register.tr();

  static const String _send = 'send';
  static String get send => _send.tr();

  static const String _product = 'product';
  static String get product => _product.tr();

  static const String _searchOnProviders = 'search_on_providers';
  static String get searchOnProviders => _searchOnProviders.tr();

  static const String _seeAll = 'see_all';
  static String get seeAll => _seeAll.tr();

  static const String _category = 'category';
  static String get category => _category.tr();

  static const String _emptyOtpRequired = 'empty_otp_required';
  static String get emptyOtpRequired => _emptyOtpRequired.tr();

  static const String _passValidation = 'pass_validation';
  static String get passValidation => _passValidation.tr();

  static const String _passRequiredValidation = 'pass_required_validation';
  static String get passRequiredValidation => _passRequiredValidation.tr();

  static const String _mailValidation = 'mail_validation';
  static String get mailValidation => _mailValidation.tr();

  static const String _phoneValidation = 'phone_validation';
  static String get phoneValidation => _phoneValidation.tr();

  static const String _confirmValidation = 'confirm_validation';
  static String get confirmValidation => _confirmValidation.tr();

  static const String _fillField = 'fill_field';
  static String get fillField => _fillField.tr();

  static const String _operationFaild = 'operation_faild';
  static String get operationFaild => _operationFaild.tr();

  static const String _progressFaild = 'progress_faild';
  static String get progressFaild => _progressFaild.tr();

  static const String _intenetWeakness = 'intenet_weakness';
  static String get intenetWeakness => _intenetWeakness.tr();

  static const String _serverError = 'server_error';
  static String get serverError => _serverError.tr();

  static const String _checkInternet = 'check_internet';
  static String get checkInternet => _checkInternet.tr();

  static const String _checkIn = 'check_in';
  static String get checkIn => _checkIn.tr();

  static const String _checkOut = 'check_out';
  static String get checkOut => _checkOut.tr();

  static const String _writeYourFeedback = 'write_your_feedback';
  static String get writeYourFeedback => _writeYourFeedback.tr();

  static const String _pleaseWriteYourFeedback = 'please_write_your_feedback';
  static String get pleaseWriteYourFeedback => _pleaseWriteYourFeedback.tr();

  static const String _rateNow = 'rate_now';
  static String get rateNow => _rateNow.tr();

  static const String _errorExeptionNoconnection = 'error_exeption_noconnection';
  static String get errorExeptionNoconnection => _errorExeptionNoconnection.tr();

  static const String _errorexeptionNointernetdesc = 'errorexeption_nointernetdesc';
  static String get errorexeptionNointernetdesc => _errorexeptionNointernetdesc.tr();

  static const String _errorexceptionNotcontain = 'errorexception_notcontain';
  static String get errorexceptionNotcontain => _errorexceptionNotcontain.tr();

  static const String _errorexceptionNotcontaindesc = 'errorexception_notcontaindesc';
  static String get errorexceptionNotcontaindesc => _errorexceptionNotcontaindesc.tr();

  static const String _visitorText = 'visitor_text';
  static String get visitorText => _visitorText.tr();

  static const String _exceptionError = 'exception_error';
  static String get exceptionError => _exceptionError.tr();

  static const String _excpetionErrorDesc = 'excpetion_error_desc';
  static String get excpetionErrorDesc => _excpetionErrorDesc.tr();

  static const String _cancel = 'cancel';
  static String get cancel => _cancel.tr();

  static const String _photoLibrary = 'photo_library';
  static String get photoLibrary => _photoLibrary.tr();

  static const String _noReportsRightNow = 'no_reports_right_now';
  static String get noReportsRightNow => _noReportsRightNow.tr();

  static const String _camera = 'camera';
  static String get camera => _camera.tr();

  static const String _badRequest = 'bad_request';
  static String get badRequest => _badRequest.tr();

  static const String _unauthorized = 'unauthorized';
  static String get unauthorized => _unauthorized.tr();

  static const String _notFound = 'not_found';
  static String get notFound => _notFound.tr();

  static const String _name = 'name';
  static String get name => _name.tr();

  static const String _continueAsA = 'continue_as_a';
  static String get continueAsA => _continueAsA.tr();

  static const String _visitor = 'visitor';
  static String get visitor => _visitor.tr();

  static const String _search = 'search';
  static String get search => _search.tr();

  static const String _noDataFound = 'no_data_found';
  static String get noDataFound => _noDataFound.tr();

  static const String _startNow = 'start_now';
  static String get startNow => _startNow.tr();

  static const String _phoneNumber = 'phone_number';
  static String get phoneNumber => _phoneNumber.tr();

  static const String _phoneNumberCanNotBeEmpty = 'phone_number_can_not_be_empty';
  static String get phoneNumberCanNotBeEmpty => _phoneNumberCanNotBeEmpty.tr();

  static const String _fullNameShouldBeThreeAtLeast = 'full_name_should_be_three_at_least';
  static String get fullNameShouldBeThreeAtLeast => _fullNameShouldBeThreeAtLeast.tr();

  static const String _cannotSelectAttachmentsExceptPdf = 'cannot_select_attachments_except_pdf';
  static String get cannotSelectAttachmentsExceptPdf => _cannotSelectAttachmentsExceptPdf.tr();

  static const String _doYouHaveTaxNumber = 'do_you_have_tax_number';
  static String get doYouHaveTaxNumber => _doYouHaveTaxNumber.tr();

  static const String _didNotGetVerifyNumber = 'did_not_get_verify_number';
  static String get didNotGetVerifyNumber => _didNotGetVerifyNumber.tr();

  static const String _resendCode = 'resend_code';
  static String get resendCode => _resendCode.tr();

  static const String _changePassword = 'change_password';
  static String get changePassword => _changePassword.tr();

  static const String _currentPassword = 'current_password';
  static String get currentPassword => _currentPassword.tr();

  static const String _changedSuccessfully = 'changed_successfully';
  static String get changedSuccessfully => _changedSuccessfully.tr();

  static const String _confirmNewPassword = 'confirm_new_password';
  static String get confirmNewPassword => _confirmNewPassword.tr();

  static const String _pleaseEnterYourCurrentPassword = 'please_enter_your_current_password';
  static String get pleaseEnterYourCurrentPassword => _pleaseEnterYourCurrentPassword.tr();

  static const String _pleaseEnterYourNewPassword = 'please_enter_your_new_password';
  static String get pleaseEnterYourNewPassword => _pleaseEnterYourNewPassword.tr();

  static const String _pleaseEnterYourConfirmPassword = 'please_enter_your_confirm_password';
  static String get pleaseEnterYourConfirmPassword => _pleaseEnterYourConfirmPassword.tr();

  static const String _thereIsErrorOccurs = 'there_is_error_occurs';
  static String get thereIsErrorOccurs => _thereIsErrorOccurs.tr();

  static const String _wantToLogOut = 'want_to_log_out';
  static String get wantToLogOut => _wantToLogOut.tr();

  static const String _areYouSureWantToLogout = 'are_you_sure_want_to_logout';
  static String get areYouSureWantToLogout => _areYouSureWantToLogout.tr();

  static const String _enableNotification = 'enable_notification';
  static String get enableNotification => _enableNotification.tr();

  static const String _deleteMyAccount = 'delete_my_account';
  static String get deleteMyAccount => _deleteMyAccount.tr();

  static const String _wantToDeleteYourAccount = 'want_to_delete_your_account';
  static String get wantToDeleteYourAccount => _wantToDeleteYourAccount.tr();

  static const String _changePhone = 'change_phone';
  static String get changePhone => _changePhone.tr();

  static const String _changeEmail = 'change_email';
  static String get changeEmail => _changeEmail.tr();

  static const String _newEmail = 'new_email';
  static String get newEmail => _newEmail.tr();

  static const String _pleaseEnterYourNewEmail = 'please_enter_your_new_email';
  static String get pleaseEnterYourNewEmail => _pleaseEnterYourNewEmail.tr();

  static const String _verifyEmailOwnership = 'verify_email_ownership';
  static String get verifyEmailOwnership => _verifyEmailOwnership.tr();

  static const String _areYouSureYouWantToDeleteAccount = 'are_you_sure_you_want_to_delete_account';
  static String get areYouSureYouWantToDeleteAccount => _areYouSureYouWantToDeleteAccount.tr();

  static const String _pleaseEnterTheVerificationCodeSentToYourEmail = 'please_enter_the_verification_code_sent_to_your_email';
  static String get pleaseEnterTheVerificationCodeSentToYourEmail => _pleaseEnterTheVerificationCodeSentToYourEmail.tr();

  static const String _currentPhone = 'current_phone';
  static String get currentPhone => _currentPhone.tr();

  static const String _newPhone = 'new_phone';
  static String get newPhone => _newPhone.tr();

  static const String _password = 'password';
  static String get password => _password.tr();

  static const String _selectLanguage = 'select_language';
  static String get selectLanguage => _selectLanguage.tr();

  static const String _pleaseEnterYourPhoneNumber = 'please_enter_your_phone_number';
  static String get pleaseEnterYourPhoneNumber => _pleaseEnterYourPhoneNumber.tr();

  static const String _pleaseEnterYourPassword = 'please_enter_your_password';
  static String get pleaseEnterYourPassword => _pleaseEnterYourPassword.tr();

  static const String _verify = 'verify';
  static String get verify => _verify.tr();

  static const String _phoneNumberVerification = 'phone_number_verification';
  static String get phoneNumberVerification => _phoneNumberVerification.tr();

  static const String _pleaseEnterTheVerificationCodeSentToYourMobilePhone = 'please_enter_the_verification_code_sent_to_your_mobile_phone';
  static String get pleaseEnterTheVerificationCodeSentToYourMobilePhone => _pleaseEnterTheVerificationCodeSentToYourMobilePhone.tr();

  static const String _requestActiveCode = 'request_active_code';
  static String get requestActiveCode => _requestActiveCode.tr();

  static const String _termsAndConditionsForUsers = 'terms_and_conditions_for_users';
  static String get termsAndConditionsForUsers => _termsAndConditionsForUsers.tr();

  static const String _remainingTime = 'remaining_time';
  static String get remainingTime => _remainingTime.tr();

  static const String _forgotPassword = 'forgot_password';
  static String get forgotPassword => _forgotPassword.tr();

  static const String _newPassword = 'new_password';
  static String get newPassword => _newPassword.tr();

  static const String _confirmPassword = 'confirm_password';
  static String get confirmPassword => _confirmPassword.tr();

  static const String _doNotHaveAnAccount = 'do_not_have_an_account';
  static String get doNotHaveAnAccount => _doNotHaveAnAccount.tr();

  static const String _createAccount = 'create_account';
  static String get createAccount => _createAccount.tr();

}
